...
- **Auth** vive em [src/hooks/useAuth.tsx](src/hooks/useAuth.tsx) (sessão + linha de `profiles`); sem sessão, `/login`.
- **Acesso é por PERFIL, não por cargo** (desde 10/09/2026). `profiles.cargo` é texto livre e serve só de rótulo na tela; quem decide o que a pessoa vê é `profiles.perfil`, uma lista **fechada** de oito valores. Tudo mora em [src/lib/modules.ts](src/lib/modules.ts): os perfis (`PERFIS`), as capacidades, e o `PORTAO` — a lista prefixo-de-rota → capacidade de onde saem **tanto** o filtro do menu (`gruposVisiveis` em [navegacao.ts](src/lib/navegacao.ts), que serve barra lateral e ⌘K) **quanto** a trava da rota digitada (`podeVerRota`, aplicada no [AppLayout](src/components/AppLayout.tsx) e no [MobileLayout](src/components/mobile/MobileLayout.tsx)). Nunca escreva a regra de acesso em dois lugares: tela que some do menu e continua respondendo pela URL é pior que tela aberta, porque parece protegida.
  - **A matriz é DADO, não constante.** A tabela `acesso_perfil` (perfil → `capacidades[]`) é editada em **Configurações › Usuários › Perfis de acesso** ([PerfisAcesso.tsx](src/pages/usuarios/PerfisAcesso.tsx), lógica pura em [matriz.ts](src/pages/usuarios/matriz.ts)). `PERFIS` no código é o **padrão** de quem não tem linha lá e o fallback se a leitura falhar — errar para a matriz revisada do repo é melhor que trancar tudo (vazio) ou abrir tudo. Quem carrega é o `AuthProvider`, e o acesso pronto sai de **`useAcesso()`**; nunca chame `acessoDe(profile)` solto numa tela, ou ela usa o padrão e ignora a edição.
  - **O `admin` não se edita**, nem pela tela nem por PATCH: `acessoDe` ignora a matriz para ele e o trigger `acesso_perfil_guarda` reescreve a linha cheia. Sem isso dava para tirar `usuarios` do admin e deixar a própria tela inalcançável.
  - **O padrão é o MÍNIMO.** Perfil nulo ou desconhecido vira `restrito`, que não abre nada (mostra [SemAcesso](src/components/SemAcesso.tsx)). Antes era o contrário — cargo não reconhecido ganhava o Hub inteiro, e sete contas estavam assim.
  - **Tela nova precisa de entrada no `PORTAO`.** Sem ela a rota fica livre para os oito perfis, em silêncio. `src/lib/modules.test.ts` cobre os itens de menu e falha se aparecer uma tela órfã, mas não adivinha rota fora do menu.
  - **Esconder ≠ proteger, e o buraco maior são as RPCs.** O Hub tem **199 funções `security definer`**, todas de `postgres` (`rolbypassrls`), e só ~43 checam algo — elas **furam qualquer RLS por construção**. Trancar tabela sem trancar a RPC não fecha nada: `demonstracoes_lancamentos` devolvia o razão inteiro a qualquer sessão logada.
  - **As três peças da trava** (migration `20260910230000`): `pode(cap)` lê a mesma matriz da tela; `pode_ler(cap)` é a versão **STABLE e sem escrita**, para policy (o PostgREST abre transação read-only em GET, e a policy roda por linha — um INSERT ali derruba a consulta); `exigir(cap, onde)` é a de RPC, que registra e devolve boolean, servindo tanto para `if not exigir(...) then return;` quanto para `where exigir(...)` em função `language sql`.
  - **`acesso_modo` é o interruptor por capacidade:** `aviso` registra em `acesso_negado` e deixa passar; `bloqueio` recusa. **Vire para `bloqueio` só depois de olhar `acesso_negado_resumo`**, que aparece na própria tela de Perfis de acesso. Capacidade sem linha vale `aviso` — nada nasce fechando sozinho. Em bloqueio hoje: `societario`, `remuneracao`, `assistente`.
  - **Para trancar uma RPC, EMBRULHE, não reescreva.** `alter function … rename to <nome>_interno`, revogue a interna de `authenticated`, e crie a casca com a mesma assinatura chamando `exigir()` antes de delegar (ver `demonstracoes_lancamentos`). Reescrever 60 linhas de SQL com regex e `unaccent` só para inserir um `if` é o jeito mais fácil de introduzir um bug de cálculo numa migration de segurança.
  - **`node scripts/mapa-acesso.mjs`** diz quais capacidades leem cada tabela e cada RPC, cruzando o `.from(...)`/`.rpc(...)` de cada arquivo com o `PORTAO`. Rode antes de fechar qualquer coisa: 34 tabelas são lidas por mais de uma capacidade (`demonstracoes_contabeis` por quatro — Demonstrações, Apresentações, Dashboard e BP), e fechar por palpite deixa tela vazia sem erro nenhum.
  - **No servidor, a trava é `requireUser(req, { exigirPerfis: [...] })`** ([_shared/auth.ts](supabase/functions/_shared/auth.ts)). `create-user`, `delete-user` e `admin-reset-password` exigem `admin`; o `ai-chat` chama `pode_usar_assistente()`, porque o system prompt dele injeta o negócio inteiro e esconder a bolinha no front não fecha nada.
...
- **O teto de concorrência de um `_shared/*` é por ISOLATE, e o limite da API é por CONTA.** Cada função tem a sua cópia do módulo, logo o seu próprio portão: `TETO_CONCORRENTE = 8` no [_shared/asaas.ts](supabase/functions/_shared/asaas.ts) vira 24 requisições simultâneas quando três crons disparam juntos. **Dois crons que varrem o mesmo endpoint não podem cair no mesmo minuto** — e a migration `20260903250000` pôs cinco no mesmo, com três em `/payments`. Resultado medido em 04–11/09/2026: `asaas-sync-diario` 17 `IDLE_TIMEOUT (150s)` em 24 rodadas, `estornos-sync` 22/24, `asaas-janela` 13/24, e a `asaas-extrato-sync` — a única que não pagina `/payments` — 0/24, no mesmo minuto. As falhas vinham em bloco, que é a assinatura de disputa e não de função lenta. Conserto em três partes (`20260911200000`): **espaçar** os horários (7 min, acima do pior caso de 2,5 min); **não duplicar** trabalho entre crons (o `atualizar` refazia inteira a janela que o cron da janela já fazia no mesmo instante); e dar **orçamento** à rodada. Quando desconfiar de disputa, cruze `automacao_execucao` por slot: se cai junto, é recurso compartilhado.
- **Rodada parcial se responde 200; volta que não se fecha se responde 500.** Trocar o 504 por um 200 sem essa segunda metade não conserta nada — só apaga o aviso. O padrão dos dois (`SEM_RODADA_COMPLETA_H` na asaas-sync, `ultima_volta_completa` na estornos-sync): cada puxada grava o que traz antes de terminar, então o relógio corta frescor e nunca consistência; o que sobrou segue em `EdgeRuntime.waitUntil` e a rodada seguinte alcança. E **um erro numa puxada não pode derrubar as outras** — `Promise.all` rejeitando inteiro descartava o recálculo de tudo que já estava gravado.
