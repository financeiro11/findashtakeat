 ...
 *
 * `folha_depara`, `folha_ajustes_log` e `folha_envios_omie` nasceram depois da
 * última geração de tipos, então o cliente tipado do Supabase não as conhece e
 * `supabase.from("folha_depara")` não compila.
 *
...
type Consulta = {
  select: (colunas: string) => Resposta<Record<string, unknown>[]> & {
    eq: (coluna: string, valor: string) => Consulta["select"] extends never ? never : {
...
export type TabelaDaFolha = "folha_depara" | "folha_ajustes_log" | "folha_envios_omie";
