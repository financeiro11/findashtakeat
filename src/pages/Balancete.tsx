...
import type { BalanceteData, BalanceteTotals } from "./balancete/types";
import { isV2, previousPeriodo } from "./balancete/utils";
...
export default function Balancete() {
  ...
  const [periodo, setPeriodo] = useState<string>(periodoMes(today));
  ...
  useEffect(() => {
    document.title = "FinHub · Balancete";
  }, []);

  // Ao abrir a tela, mostra direto o ÚLTIMO arquivo salvo (mais recentemente importado
  // com dados de verdade) em vez do mês atual — que normalmente ainda está vazio.
  useEffect(() => {
    (async () => {
      const ultimo = await buscarPeriodoMaisRecenteComDados("balancete");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (isV2(dados)) {
        ...
        setData(null);
        setStatus(path ? "empty" : "empty");
      }

      // mês anterior
      const prev = previousPeriodo(periodo);
  ...
  const reprocess = useCallback(
    async (path?: string) => {
      ...
      try {
        ...
        const r = await aguardarExtracao("balancete", periodo);
        if (!r.ok) throw new Error((r as { ok: false; error: string }).error);
        toast.success(`Balancete processado (${r.contas} contas)`);
        await load();
      } catch (err: any) {
        setStatus("error");
        setErrorMsg(err.message);
  ...
  const importPdf = useCallback(
    async (file: File) => {
      try {
        const path = `balancete/${periodo}-${Date.now()}.pdf`;
        ...
        const { error } = await supabase.from("demonstracoes_contabeis").upsert(
          {
            ...
            periodo,
            pdf_path: path,
            dados: (data as any) ?? { version: 2, kind: "balancete", accounts: [], totals: {}, source: "pdf", imported_at: new Date().toISOString() },
          } as any,
  ...
  const clear = useCallback(async () => {
    if (!confirm(`Excluir balancete de ${periodo}?`)) return;
    try {
  ...
  return (
    <div className="space-y-4 p-5">
      ...
      {loading ? (
        ...
        <>
          <BalanceteKpis totals={totals} prevTotals={prevData?.totals ?? null} loading={false} />
          <BalanceteTable accounts={accounts} prevAccounts={prevAccounts} />
          <BalanceteCharts accounts={accounts} history={history} />
        </>
      )}
  ...
  return (
    <div className="card-surface p-12 flex flex-col items-center justify-center gap-3 text-center">
      ...
      <div className="text-lg font-semibold text-foreground">
        {processing
          ? "Processando o balancete com IA..."
          : hasPdf
          ? "PDF anexado, mas ainda não processado"
          : "Nenhum balancete importado neste mês"}
      </div>
      <div className="text-sm text-muted-foreground max-w-md">
        {processing
          ? "A IA está lendo o PDF e estruturando as contas. Isso leva alguns segundos."
          : "Envie o PDF do balancete enviado pela contabilidade. O sistema lê automaticamente e converte em uma análise financeira interativa."}
      </div>
    </div>
  );
}
