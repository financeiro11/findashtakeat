...
import { RefreshCw, Loader2, MessageCircle, Eye, EyeOff } from "lucide-react";
...
export default function Caixa() {
  ...
  const projData = useMemo(() => {
    ...
    return pts.map((pt) => {
      ...
      return { data: fmtDiaMes(pt.data), saldo, cor: pt.data === maiorData && pt.saidas > 0 ? "maior" : saldo >= atual ? "acima" : "abaixo" };
    });
  }, [snap, contasView]);
    ...
    <div className="space-y-4 p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
        <SectionCard
          ...
          actions={<span className="text-[11px] text-muted-foreground">{Math.min(p!.movimentacoes.length, p!.mov_total)} de {p!.mov_total} lançamentos</span>}
          padded={false}
        >
        ...
        <SectionCard title="Fluxo de caixa projetado · próximos 21 dias" subtitle="Saldo diário estimado a partir dos títulos a pagar e a receber do Omie">
          <div className="mb-2">
          ...
          <div className="h-[190px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={projData} margin={{ top: 4, right: 4, left: 4, bottom: 0 }}>
                ...
                <Tooltip formatter={(v: any) => [fmtBRL(Number(v)), "Saldo"]} labelClassName="text-[11px]" contentStyle={{ fontSize: 11, borderRadius: 8 }} />
