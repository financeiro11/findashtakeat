// Aba "Churn" de /assinaturas — o churn do Takeat OS, lido de `churn_snapshot`.
//
// Desde 18/09/2026 quem escreve `churn_snapshot` é `os_churn_para_snapshot()`, no fim da
// cópia diária do OS (migration 20260918160000); a planilha de churn saiu de cena. O OS não
// tem quebra por qualificação nem setor Comercial, e desde abr/26 não traz a QUANTIDADE do
// churn de Onboarding — ela é estimada (% × base) e vem listada em `dados.estimados`.
//
// A base do mês M é o MRR de assinatura e os clientes de M−1 (`os_assinaturas`), o mesmo
// denominador que o OS usa nos percentuais.
...
/* ------------------------------ tipos ------------------------------ */
type LinhaSetor = {
  setor: string; qtd: number | null; valor: number;
  pct_do_cancelamento: number; pct_receita: number | null; pct_receita_liquido: number | null; pct_cliente: number | null;
};
...
type Kpis = {
  ...
  downsell_qtd: number | null; downsell_valor: number;
  upsell_qtd: number | null; upsell_valor: number;
  reativacao_qtd: number; reativacao_valor: number;
  churn_qtd: number; churn_valor: number; churn_tm: number;
  pct_receita_geral: number; pct_receita_onboarding: number | null; pct_receita_sucesso: number | null;
  pct_cliente_geral: number; pct_cliente_onboarding: number | null; pct_cliente_sucesso: number | null;
  /** Meta do OS (orçado de "% Churn Revenue Total"); null nos meses sem meta lançada. */
  meta_pct: number | null; meta_valor: number | null; north_star: number | null;
};
type Dados = {
  competencia: string; mes_label: string; mes_nome: string; mes_num: number;
  base: {
    mrr_inicio: number; clientes: number; tm_mrr: number;
    /** true quando o mês não tem linha em os_assinaturas e a base saiu do % do próprio OS. */
    derivada?: boolean;
  ...
  /** O que foi estimado: "onboarding_qtd" (% × base) e/ou "base" (derivada do %). */
  estimados?: string[];
};
...
export default function Churn({ aba, setAba }: { aba: Aba; setAba: (a: Aba) => void }) {
  ...
  const historico = useMemo(() => snaps.map((s, i) => {
    ...
    return {
      ...
      meta_pct: s.dados?.kpis?.meta_pct ?? null,
      north_star: s.dados?.kpis?.north_star ?? null,
      ...
      <div className="space-y-4 p-4 md:p-6">
        ...
        <div className="card-surface mx-auto mt-10 max-w-md p-8 text-center">
          ...
          <p className="text-[12.5px] text-muted-foreground">
            Os números vêm do Takeat OS e são remontados todo dia às 6h, na cópia do OS para o Hub.
            Se estiver vazio, a cópia não rodou — veja em Configurações › Monitoramento.
          </p>
        </div>
      </div>
    );
  ...
  // O OS não quebra o cancelamento por porte: sem nenhum nível em nenhum recorte, o bloco some.
  const temQualificacao = (d.por_qualificacao?.geral?.length ?? 0) > 0;
  const conf = d.base.conferencia;
  const estOnb = !!d.estimados?.includes("onboarding_qtd");
  const pctOuTraco = (v: number | null | undefined) => (v == null ? "—" : fmtPct2(v));
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <KpiCard
          eyebrow="# Churn (clientes)" valor={<>{estOnb && <span title="Inclui a quantidade de Onboarding estimada (% do OS × base)">≈</span>}{fmtInt(k.churn_qtd)}</>}
        ...
        />
        <KpiCard
          eyebrow="North Star Metric" valor={pctOuTraco(k.north_star)}
          delta={k.north_star == null ? null : delta(k.north_star, kPrev?.north_star ?? undefined)}
          icon={<Target className="h-4 w-4" />}
          rodape={k.meta_pct == null ? <>Sem meta lançada no OS para este mês</> : <>Meta OS {fmtPct2(k.meta_pct)} · {fmtCheio(k.meta_valor ?? 0)} tolerado</>}
          extra={
            orc?.churn_pct == null ? (
              ...
              <>
                ...
                {" "}— o plano tolera mais churn do que a meta do OS
              </>
            )
          }
        />
      ...
      <div className="card-surface p-4">
          ...
          {d.base.snapshot_recorrencia && (
            <button
              ...
              className={cn(
                ...
                !conf || conf.confere ? "border-border text-muted-foreground" : "border-amber-500/40 text-amber-600 dark:text-amber-400",
              )}
              title={
                !conf
                  ? "A base do mês é o MRR de assinatura e os clientes do mês anterior, no Takeat OS."
                  : conf.confere
                  ? "O MRR de início do mês é idêntico ao MRR Core da aba Recorrência."
                  : `Diferença de ${fmt2(Math.abs(conf.diferenca))} entre as duas fontes.`
              }
            >
              {!conf || conf.confere ? <Check className="h-3 w-3" /> : <AlertTriangle className="h-3 w-3" />}
        ...
        <div className="mt-3 flex flex-wrap items-baseline gap-x-4 gap-y-1 rounded-lg border border-border bg-muted/30 p-3">
          ...
          {d.base.derivada && (
            <span className="text-[11.5px] text-amber-600 dark:text-amber-400"
              title="O OS não tem a carteira deste mês; a base saiu de valor ÷ % do próprio OS (erro de arredondamento ~0,3%).">
              base derivada do % do OS
            </span>
          )}
          {conf && !conf.confere && (
            <span className="text-[11.5px] text-amber-600 dark:text-amber-400">
        ...
        <div className="mt-3 space-y-2">
          <LinhaCascata rotulo="Cancelamentos" sinal="+" qtd={k.cancel_qtd} estimada={estOnb} valor={k.cancel_valor} max={k.cancel_valor} cor="bg-neg/70" mrr={d.base.mrr_inicio} />
        ...
        <p className="mt-2 text-[11px] text-muted-foreground">
          Fórmula do OS: churn de Onboarding + (cancelamentos do Sucesso + downsell − upsell − reativações).
          O OS não conta quantos downsells e upsells houve, só o valor.
          {estOnb && " ≈ A quantidade de Onboarding é estimada: o OS lança só o %, e a conta usa % × clientes da base."}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-5">
        {/* por setor */}
        <div className={cn("card-surface p-4", temQualificacao ? "lg:col-span-2" : "lg:col-span-5")}>
              ...
              <div key={s.setor}>
                <div className="flex items-center justify-between text-[12.5px]">
                  ...
                  <span className="text-muted-foreground">
                    {s.qtd == null ? "—" : <>{estOnb && chave(s.setor) === "onboarding" && "≈"}{fmtInt(s.qtd)}</>} · <span className="num font-semibold text-foreground">{fmtK(s.valor)}</span>
                  </span>
                </div>
                ...
                <div className="mt-1 flex items-center justify-between text-[10.5px] text-muted-foreground">
                  ...
                  <span className="num">{pctOuTraco(s.pct_receita)} da receita · {pctOuTraco(s.pct_cliente)} da base</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 border-t border-border/50 pt-3">
            <div>
              ...
              <div className="num text-[15px] font-semibold text-foreground">{pctOuTraco(k.pct_receita_onboarding)}</div>
              <div className="text-[10.5px] text-muted-foreground">{pctOuTraco(k.pct_cliente_onboarding)} dos clientes</div>
            </div>
            <div>
              <div className="eyebrow">% Churn receita — Sucesso</div>
              <div className="num text-[15px] font-semibold text-foreground">{pctOuTraco(k.pct_receita_sucesso)}</div>
              <div className="text-[10.5px] text-muted-foreground">líquido de upsell e reativações</div>
            </div>
          </div>
        </div>

        {/* por qualificação — só existe se a fonte trouxer o porte do cliente que cancelou */}
        {temQualificacao && (
      ...
      <div className="card-surface p-4">
        ...
        {histView === "mes" ? (
          <div className="mt-3 overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-border text-[10px] uppercase tracking-wider text-muted-foreground">
                  ...
                  <th className="px-2 py-1.5 text-right font-bold">Meta OS</th>
              ...
              <tbody>
                {historico.map((h) => (
                  ...
                  >
                    ...
                    <td className={cn("num px-2 py-1.5 text-right text-[12.5px] font-semibold", h.meta_pct == null ? "text-foreground" : h.pct_receita <= h.meta_pct ? "text-pos" : "text-neg")}>
                      {fmtPct2(h.pct_receita)}
                    </td>
                    <td className="num px-2 py-1.5 text-right text-[12px] text-muted-foreground">{h.meta_pct == null ? "—" : fmtPct2(h.meta_pct)}</td>
                    ...
                    <td className={cn("num px-2 py-1.5 text-right text-[12.5px]", (h.north_star ?? 0) >= 100 ? "text-pos" : "text-muted-foreground")}>
                      {h.north_star == null ? "—" : fmtPct1(h.north_star)}
                    </td>
            ...
            <p className="mt-2 text-[11px] text-muted-foreground">
              Clique num mês para carregar a competência. “Parcial” = mês ainda em andamento no OS.
              A <b>meta OS</b> é o orçado de “% Churn Revenue Total” no Takeat OS; o <b>orçado BP</b> é o “% Churn Mensal” da aba Operação —
          ...
          <div className="mt-3">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-4 text-[10.5px] text-muted-foreground">
              ...
              <span className="flex items-center gap-1.5"><span className="h-0.5 w-4 rounded bg-muted-foreground" /> Meta OS</span>
...
/* ------------------------------ subcomponentes ------------------------------ */
function Cabecalho({ aba, setAba, nav }: {
    ...
    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
      ...
      <div className="flex shrink-0 items-center gap-2 self-start">
        ...
        <span className="inline-flex items-center gap-1.5 rounded-md bg-primary px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-primary-foreground"
          title="Remontado todo dia às 6h, na cópia do Takeat OS para o Hub">
          <Database className="h-3 w-3" /> Takeat OS
        </span>
      </div>
    </div>
  );
...
function LinhaCascata({
  rotulo, sinal, qtd, valor, max, cor, mrr, estimada,
}: {
  rotulo: string; sinal: "+" | "−"; qtd: number | null; valor: number; max: number; cor: string; mrr: number;
  /** A quantidade inclui parte estimada (Onboarding: % × base). */
  estimada?: boolean;
}) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="num w-10 shrink-0 text-right text-[12px] text-muted-foreground"
        title={qtd == null ? "O OS lança só o valor, não a quantidade" : estimada ? "Inclui a quantidade de Onboarding estimada" : undefined}>
        {qtd == null ? "—" : `${estimada ? "≈" : ""}${fmtInt(qtd)}`}
      </span>
...
function ChartTip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card px-3 py-2 text-[11px] shadow-md">
      ...
      {p.meta_pct != null && (
        <div className="flex items-center justify-between gap-6"><span className="text-muted-foreground">Meta OS</span><span className="num text-foreground">{fmtPct2(p.meta_pct)}</span></div>
      )}
      {p.bp_pct != null && (
