...
/* A DFC é uma TRANSCRIÇÃO do tracker — linha por linha, na ordem da planilha.
   O porquê de cada escolha (grupos das saídas, apelidos, linha que não existe
   no tracker e por isso não entra) está no espelho src/lib/demonstracoes-schema.ts.
   A ordem dos blocos de topo é fixa: DFC.tsx lê `DFC_SCHEMA[0..6]` por índice. */
