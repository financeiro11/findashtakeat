-- A nota em dólar acha o título, e o lançamento de cartão passa a ter nome.
--
-- Duas mudanças no casador, e as duas nascem da mesma peça: desde
-- `20260827130000` o `favorecido` de um "Lancamento Fatura Cartao" traz o
-- LOJISTA (1.895 de 2.678 títulos). Isso abre o que estava fechado.
--
-- 1. O ALVO PASSA A SE CHAMAR PELO LOJISTA. `alvos` ancorava em
--    `favorecido_cru`, que é o que o Omie diz — e o Omie diz "Lancamento Fatura
--    Cartao" em 2.678 títulos. Para toda regra que compara NOME, esses títulos
--    eram anônimos. `favorecido_cru` continua intacto na view; muda só o que o
--    casador usa para comparar.
--
-- 2. REGRA 7 — A INVOICE EM DÓLAR. `nota-ler-arquivo` já converte pela PTAX do
--    dia e guarda `valor_moeda`; aqui a comparação afrouxa para ±8% NAQUELA
--    LINHA, e só nela. A calibragem está em `20260827140000`.
--
-- POR QUE A IDENTIDADE É POR PALAVRA, e não por trigram: medido, "Cobrança da
-- HubSpot" contra "Hubspot Inc.V" dá similaridade 0,31, e "Datadog Billing
-- (billing@datadoghq.com)" contra "DATADOG, INC.V" dá 0,28. O corte da regra 6
-- é 0,55; baixá-lo até alcançar essas duas deixaria passar quase qualquer par.
-- Um token de 5+ letras em comum é mais preciso E mais fácil de explicar para
-- quem confere: "casou porque as duas dizem HUBSPOT".
--
-- A LISTA DE PALAVRAS QUE NÃO CONTAM é o que faz a regra valer. Sem ela,
-- "Cobrança da HubSpot" casaria com "Cobrança da Vivo" pela palavra "cobranca"
-- — e num universo de fatura de cartão, "fatura", "billing" e "invoice"
-- aparecem em quase todo nome.

create or replace function public.token_forte_em_comum(a text, b text)
returns boolean
language sql
immutable
as $$
  select exists (
    select 1
      from unnest(string_to_array(public.normaliza_nome(coalesce(a, '')), ' ')) w
     where length(w) >= 5
       and w <> all (array[
         'cobranca','cobrancas','fatura','faturas','billing','invoice','pagamento',
         'recibo','nota','notas','fiscal','servico','servicos','tecnologia','digital',
         'comercio','industria','brasil','brazil','ltda','sociedade','limitada',
         'sistemas','solucoes','consultoria','participacoes','empreendimentos'
       ])
       and public.normaliza_nome(coalesce(b, '')) like '%' || w || '%'
  );
$$;

comment on function public.token_forte_em_comum(text, text) is
  'Duas contrapartes compartilham uma palavra de 5+ letras que não é palavra de cobrança? É o teste de identidade para nome que o trigram não alcança — "Cobrança da HubSpot" x "Hubspot Inc.V" dá 0,31 de similaridade e é obviamente o mesmo fornecedor.';
