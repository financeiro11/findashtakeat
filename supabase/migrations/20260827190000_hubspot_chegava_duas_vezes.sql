-- Por que as notas do HubSpot estavam no e-mail e o Hub não as usava.
--
-- Elas estavam TODAS lá — 29 linhas, todas com arquivo. O que as impedia eram
-- três defeitos empilhados, e nenhum deles era "não achei".
--
-- ---------------------------------------------------------------------------
-- 1. O TOTAL ZERADO DA FATURA JÁ PAGA  (consertado em `nota-fiscal.ts`)
--
-- A fatura do HubSpot escreve, nesta ordem:
--
--     Total devido US$ 0,00        Valor pago (USD) US$ 5.693,73
--
-- Procurar "total" e aceitar o primeiro devolve ZERO — e zero vira nulo, então
-- a nota entrava muda no acervo com o valor impresso na cara dela. Os rótulos
-- agora são percorridos em ordem de preferência e os que dão zero são pulados:
-- numa fatura paga o número é o "valor pago"; numa em aberto, o "total devido".
--
-- ---------------------------------------------------------------------------
-- 2. A IDENTIDADE DO DOCUMENTO NUNCA VALEU PARA QUEM NÃO TEM CNPJ
--
-- `documento_chave` tinha como terceira alternativa:
--
--     nullif(regexp_replace(coalesce(cnpj,''),'\D','','g'),'')
--       || '|' || valor || '|' || enviado_em
--
-- Em SQL, `NULL || texto` é **NULL**. Sem CNPJ, a expressão inteira virava nula
-- e a alternativa era pulada — exatamente para o documento sem CNPJ, que é
-- quem mais precisava dela. Fornecedor estrangeiro não tem CNPJ e não tem chave
-- de acesso: caía sempre no último recurso, `chave`, que é única por LINHA.
-- Ou seja, duas cópias do mesmo papel contavam como dois documentos.
--
-- ---------------------------------------------------------------------------
-- 3. E POR ISSO A GUARDA DE DISPUTA MATAVA TODAS
--
-- O HubSpot manda a mesma fatura DUAS vezes — por e-mail (`gmail-nf-sync`) e
-- pela pasta "0. Gmail" do Drive (`comprovantes-drive-sync`) — e manda INVOICE
-- e RECEIPT do mesmo pagamento. Quatro linhas, mesmo valor, mesmo título.
-- Contadas como quatro documentos distintos, a guarda concluía "disputa" e
-- ninguém levava: 6 dos 7 títulos do HubSpot ficavam sem nota com a nota ali.
--
-- O conserto é reconhecer o papel pelo NÚMERO DO DOCUMENTO no nome do arquivo:
-- `HubSpot-INVOICE-793472891.0.pdf` e `2026-07-19_HubSpot-INVOICE-793472891.0.pdf`
-- são o mesmo documento; o RECEIPT-48924964 é outro. Com isso a disputa passa a
-- ser entre a invoice (peso 2, é nota) e o recibo (peso 0, não é) — e o peso já
-- sabia resolver isso desde 26/08.
--
-- O número vai com o VALOR junto na chave: número de documento não é único
-- entre fornecedores, e sem o valor duas notas de emissores diferentes com o
-- mesmo número colapsariam numa só.

create or replace function public.numero_do_documento(txt text)
returns text
language sql
immutable
as $$
  select m[1]
    from regexp_matches(coalesce(txt, ''), '(\d{6,})', 'g') m
   order by length(m[1]) desc, m[1]
   limit 1;
$$;

comment on function public.numero_do_documento(text) is
  'O maior número de 6+ dígitos do texto — o número da invoice/recibo no nome do arquivo. É a identidade do papel quando não há chave fiscal: o documento estrangeiro chega duas vezes (e-mail e Drive) e sem isto conta como dois.';
  ...
  regra as (
    ...
    select n.id, a.tipo, a.id_unico, 'cambio', 'media', 7,
      ...
      join alvos a on a.valor between n.valor * 0.92 and n.valor * 1.08
                  and a.data between n.data_ref - 20 and n.data_ref + 45
