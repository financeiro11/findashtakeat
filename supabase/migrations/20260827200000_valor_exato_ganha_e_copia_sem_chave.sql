-- O valor exato ganha do aproximado, e a cópia sem chave fiscal colapsa.
--
-- Continuação de `20260827190000`, com os dois defeitos que sobraram no HubSpot.
--
-- ---------------------------------------------------------------------------
-- 1. A FATURA DE AGOSTO ROUBAVA O TÍTULO DE JULHO
--
-- A banda de ±8% que o câmbio exige é larga o bastante para uma fatura alcançar
-- o título do mês anterior quando o valor em dólar é o MESMO todo mês — que é
-- exatamente o caso de uma assinatura:
--
--   fatura 19/07  US$ 5.693,73 → R$ 29.138,23   (bate AO CENTAVO com o título)
--   fatura 19/08  US$ 5.693,73 → R$ 29.444,56   (bate dentro da banda, 1,05%)
--   título 11/08                 R$ 29.138,23
--
-- As duas reivindicavam o mesmo título, com o mesmo peso. Duas notas de peso
-- igual no mesmo alvo é "disputa", e a guarda matava AS DUAS — o título ficava
-- sem nota com a nota certa do lado.
--
-- O peso ganha um degrau: **valor ao centavo vale mais que valor na banda**.
-- Ele só pode ser calculado depois de conhecer o alvo, então mora em
-- `pretendentes` e não em `n`.
--
-- ---------------------------------------------------------------------------
-- 2. A MESMA NOTA SUBIRIA DUAS VEZES AO ERP
--
-- `marcar_copias` só colapsava por `chave_fiscal`. O HubSpot não tem chave — é
-- documento estrangeiro — e a mesma fatura chega por DUAS portas: o e-mail
-- (`gmail-nf-sync`) e a pasta "0. Gmail" do Drive (`comprovantes-drive-sync`).
-- Depois de `20260827190000` as duas passaram a casar com o mesmo título, o que
-- é certo; mas as duas iam para a fila, e o ERP receberia o arquivo repetido.
--
-- Agora o número do documento + valor também colapsa. O portador continua sendo
-- o de menor id, e continua herdando o que o grupo sabe.
...
declare v_n integer;
begin
  /* A IDENTIDADE DO PAPEL, em duas camadas: a chave fiscal quando existe, e o
     número do documento + valor quando não existe. A segunda é o que alcança o
     fornecedor estrangeiro, que não tem chave de acesso e manda o mesmo PDF
     pelo e-mail e pela pasta do Drive. */
  with identidade as (
    select id,
           coalesce(
             chave_fiscal,
