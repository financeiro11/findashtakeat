-- O casador passa a perguntar a cotação do lote antes de fechar a conta.
--
-- `notas_cambio_lote` (ver `20260827280000`) existe desde a migração anterior e
-- rodava só quando alguém a chamava à mão. Isso não se sustenta: `casar` REFAZ
-- o alvo de tudo que ainda não subiu ao ERP, a cada 18 minutos, e apagaria os
-- casamentos dela sem erro nenhum — o número na tela simplesmente voltaria ao
-- que era, que é o pior jeito de um defeito aparecer.
--
-- A chamada entra depois da decisão principal e ANTES do double check contra o
-- ERP, para que o que ela casar receba `conferencia` e passe pela porta da fila
-- na mesma rodada — senão a nota ficaria casada e parada até a rodada seguinte.
--
-- Abaixo, a `notas_externas_casar` inteira. Mudou uma linha; ela vem completa
-- porque é assim que a próxima pessoa lê o que está rodando.
