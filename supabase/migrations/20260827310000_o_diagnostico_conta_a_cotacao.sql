-- O diagnóstico passa a contar a cotação — para a IA ter conta, não adjetivo.
--
-- `notas_cambio_lote` (`20260827280000`) deduz a cotação de cada fatura de cartão
-- e casa invoice com título por ela. O sinal existia e não chegava a lugar
-- nenhum: a aba "Por que falta" continuava dizendo só que o título estava sem
-- nota.
--
-- O que entra é o que dá para conferir: a cotação, quantos títulos a
-- corroboram, o espalhamento entre eles e quantas notas casaram por SOMA de
-- duas invoices. Com isso a IA escreve *"os seis títulos da Datadog de 11/08 são
-- uma fatura de cartão só, convertida a 5,0742, e cinco invoices de janeiro a
-- junho casaram nela"* — uma frase que alguém pode ir conferir.
--
-- A divisão de trabalho continua a mesma do resto do repo: **a conta é do
-- Postgres, a redação é da IA**. Um modelo somando US$ 1.448,61 com US$ 1.673,95
-- erra sem avisar, e erra com uma frase convincente em volta.
--
-- Abaixo, a `cap_notas_diagnostico` inteira, com um bloco a mais no sinal.
