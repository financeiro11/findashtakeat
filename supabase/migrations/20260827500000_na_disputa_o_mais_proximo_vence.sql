-- NA DISPUTA, A GUARDA MATAVA OS DOIS LADOS — e ela ficou maior depois de o
-- desempate melhorar.
--
-- Medido em 27/08/2026, logo depois de `20260827490000`: os documentos parados
-- por `alvo_disputado` SUBIRAM de 510 para 589. Não é regressão, é consequência
-- direta: o desempate passou a dar alvo a muito mais documento, e três NFs do
-- mesmo colaborador que antes morriam separadas em `varios_alvos` agora chegam
-- juntas no mesmo título — e ali a guarda de disputa mata as três.
--
-- A guarda existe por um motivo certo: dois documentos reivindicando um título
-- significa que pelo menos um está errado, e grudar o errado é pior do que não
-- grudar nada. Mas "pelo menos um está errado" não é "todos estão errados", e
-- entre três NFs mensais de R$ 20.000 do mesmo CNPJ há uma que é claramente a
-- daquele mês: a mais próxima em data. As outras não perdem para ela por sorteio
-- — perdem por distância medida.
--
-- ---------------------------------------------------------------------------
-- O CRITÉRIO É DISTÂNCIA, E O EMPATE CONTINUA MATANDO
--
-- Antes o vencedor era `min(documento_chave)` — ordem alfabética de uma chave
-- interna, que não quer dizer nada sobre qual documento é daquele título. Ele só
-- era usado quando `docs = 1`, ou seja, nunca decidia disputa nenhuma; era um
-- desempate que não desempatava.
--
-- Agora vence quem está mais perto em data do título, e SÓ quando esse mais
-- perto é único. Dois documentos à mesma distância continuam matando o par —
-- distância igual não é prova, e escolher um ali seria o sorteio que a guarda
-- existe para impedir.
--
-- ---------------------------------------------------------------------------
-- E A FILA DRENA SOZINHA, UMA CAMADA POR RODADA
--
-- O perdedor não fica preso para sempre. Quando o vencedor sobe ao ERP, aquele
-- título deixa de estar devendo, sai da corrida do desempate na rodada seguinte
-- (`20260827490000` fez o desempate correr só entre quem ainda deve) e o
-- segundo documento acha o SEU mês. Três NFs de um colaborador levam três
-- passadas do cron das :00 e :30 para se acomodarem, cada uma no seu título.
-- Isso é melhor do que resolver tudo numa passada: cada camada é conferível
-- contra o ERP antes de a próxima acontecer.
--
-- Abaixo, a `notas_externas_casar` inteira. Mudaram `pretendentes` (que agora
-- carrega a distância), `disputa` e a linha do `ganhou`.
...
begin
  ...
  /* DOIS DESEMPATES, e os dois correm SÓ ENTRE OS TÍTULOS QUE AINDA DEVEM.
     Ver o cabeçalho de `20260827490000`: medir a distância contra todos e depois
  ...
  /* O PESO GANHA UM DEGRAU: valor que bate AO CENTAVO vale mais que valor que
     ...
     A DISTÂNCIA VEM JUNTO desde 27/08/2026: é ela que decide a disputa logo
     abaixo, e ela também só existe com o alvo na mão. */
  pretendentes as (
    select d.nota_id, d.tipo, d.id_unico, d.prio, n.documento_chave,
           abs(a.data - n.data_ref) as dist,
           n.peso + case
  ...
  /* Quem ainda disputa o título depois do peso, e a que distância.
     `min(dist)` por documento porque o mesmo papel pode chegar por duas fontes
     (e-mail e Drive) com a mesma `documento_chave`: são a mesma reivindicação,
     não duas. */
  disputa_base as (
    select p.tipo, p.id_unico, p.documento_chave, min(p.dist) as dist
      from pretendentes p
     ...
     where not exists (
             select 1 from pretendentes q
              where q.tipo = p.tipo and q.id_unico = p.id_unico
                and q.prio in (0, 4))
     group by p.tipo, p.id_unico, p.documento_chave
  ),
  disputa_perto as (
    select tipo, id_unico, min(dist) as dist_min
      from disputa_base group by tipo, id_unico
  ),
  /* NA DISPUTA, O MAIS PRÓXIMO EM DATA VENCE — e empate continua matando.
     Ver o cabeçalho desta migração: matar os dois lados custava 589 documentos,
     e entre três NFs mensais do mesmo CNPJ a daquele mês é a mais próxima, não
     um sorteio. `empatados > 1` devolve a guarda antiga: distância igual não é
     prova de nada. */
  disputa as (
    select b.tipo, b.id_unico,
           count(*) as docs,
           count(*) filter (where b.dist = m.dist_min) as empatados,
           (array_agg(b.documento_chave order by b.dist, b.documento_chave))[1] as vencedor
      from disputa_base b
      join disputa_perto m on m.tipo = b.tipo and m.id_unico = b.id_unico
     group by b.tipo, b.id_unico
  )
  update public.notas_externas nt
    ...
    cross join lateral (
      select (d.quantos = 1 or d.desempatou)
         and (
           -- ninguém mais quer este título
           coalesce(disp.docs, 1) = 1
           -- ou este documento é, sozinho, o mais próximo dele
           or (disp.empatados = 1 and disp.vencedor = n.documento_chave)
         ) as ganhou
    ) g
