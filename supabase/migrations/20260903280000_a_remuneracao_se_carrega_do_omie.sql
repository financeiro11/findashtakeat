...
create or replace view public.vw_remuneracao_omie
...
base as (
  select m.cod_titulo, m.valor, m.vencimento, m.pagamento, m.doc_mov,
...
select
  ...
  -- A COMPETÊNCIA É O MÊS DO VENCIMENTO MENOS UM.
  --
  -- Não dá para usar a emissão: ela é NULA na maioria dos títulos — só os que o
  -- Hub provisionou (ago/2026 em diante) a preenchem. Sobra o vencimento, e ele
  -- está sempre um mês à frente do mês trabalhado: o fixo vence dia 05 ou 07 e a
  -- premiação dia 15, ambos do mês SEGUINTE. O `folha_envios_omie` prova o caso
  -- do fixo (folha de agosto/2026 = competência 2026-08-01, vence 05/09), e a
  -- premiação segue o mesmo ciclo — a comissão paga em 15/08 é a de julho.
  --
  -- Sem isto, agosto inteiro cairia em setembro. O painel não daria erro; só
  -- mostraria o número certo no mês errado, que é o pior jeito de errar.
  (date_trunc('month', b.vencimento) - interval '1 month')::date as competencia,
...
where b.vencimento is not null
